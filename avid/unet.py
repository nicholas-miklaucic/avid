"""U-Net implementation for JAX.

Primary code from https://docs.kidger.site/equinox/examples/unet/, credit to Ben Wallace,
but adapted for 3D."""
...
class LinearTimeSelfAttention(eqx.Module):
    ...
    heads: int
    to_qkv: eqx.nn.Conv3d
    to_out: eqx.nn.Conv3d
    ...
    def __init__(
        ...
        self.to_qkv = eqx.nn.Conv3d(dim, hidden_dim * 3, 1, key=keys[0])
        self.to_out = eqx.nn.Conv3d(hidden_dim, dim, 1, key=keys[1])
    ...
    def __call__(self, x):
        ...
        qkv = self.to_qkv(x)
        q, k, v = rearrange(
            qkv, '(qkv heads c) h w d -> qkv heads c (h w d)', heads=self.heads, qkv=3
        )
        k = jax.nn.softmax(k, axis=-1)
        context = jnp.einsum('hdn,hen->hde', k, v)
        out = jnp.einsum('hde,hdn->hen', context, q)
        out = rearrange(out, 'heads c (h w d) -> (heads c) h w d', heads=self.heads, h=h, w=w, d=d)
        return self.to_out(out)


def upsample_3d(y, factor=2):
    C, H, W, D = y.shape
    y = jnp.reshape(y, [C, H, 1, W, 1, D, 1])
    y = jnp.tile(y, [1, 1, factor, 1, factor, 1, factor])
    return jnp.reshape(y, [C, H * factor, W * factor, D * factor])


def downsample_3d(y, factor=2):
    C, H, W, D = y.shape
    y = jnp.reshape(y, [C, H // factor, factor, W // factor, factor, D // factor, factor])
    return jnp.mean(y, axis=[2, 4, 6])
...
class ResnetBlock(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.block1_conv = eqx.nn.Conv3d(dim_in, dim_out, 3, padding=1, key=keys[1])
        self.block2_layers = [
            eqx.nn.GroupNorm(min(dim_out // 4, 32), dim_out),
            jax.nn.silu,
            eqx.nn.Dropout(dropout_rate),
            eqx.nn.Conv3d(dim_out, dim_out, 3, padding=1, key=keys[2]),
        ]
        ...
        if is_biggan:
            if self.up:
                self.scaling = upsample_3d
            elif self.down:
                self.scaling = downsample_3d
            else:
                self.scaling = None
        else:
            if self.up:
                self.scaling = eqx.nn.ConvTranspose3d(
                    dim_in,
                    dim_in,
                    kernel_size=4,
                    stride=2,
        ...
        self.res_conv = eqx.nn.Conv3d(dim_in, dim_out, kernel_size=1, key=keys[5])
    ...
    def __call__(self, x, t, *, key):
        ...
        for layer in self.block2_layers:
            # Precisely 1 dropout layer in block2_layersUnet which requires a key.
...
class UNet(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        data_channels, in_height, in_width, in_depth = data_shape
        ...
        self.final_conv_layers = [
            ...
            jax.nn.silu,
            eqx.nn.Conv3d(hidden_size, data_channels, 1, key=keys[6]),
        ]
